...
{                          
  ...
  G4double resolution = fTotalEnergyDeposit/MeV * G4RandGauss::shoot(1, 0.001); // 0.1% resolution
 
  analysisManager->FillH1(1, resolution);
  ...
  analysisManager->FillNtupleDColumn(6,resolution);

  G4int nV = event->GetNumberOfPrimaryVertex();
  analysisManager->FillNtupleDColumn(5, nV);
